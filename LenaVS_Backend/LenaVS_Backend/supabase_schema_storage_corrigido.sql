...
-- - bucket público de mídia/vídeos no Supabase Storage
-- =========================================
...
SET country_group = CASE
  ...
  WHEN lower(coalesce(country_group, '')) IN ('br', 'brasil', 'brazil') THEN 'BR'
  WHEN upper(coalesce(country_group, '')) = 'INTL' THEN 'INTL'
  WHEN lower(coalesce(country_group, '')) IN ('intl', 'international', 'internacional', 'other', 'outro') THEN 'INTL'
...
-- =========================================
-- SUPABASE STORAGE
-- Bucket público para vídeos e mídias persistentes
-- Upload recomendado via backend com service_role
-- =========================================
...
DROP POLICY IF EXISTS "Authenticated insert to videos bucket" ON storage.objects;
CREATE POLICY "Authenticated insert to videos bucket"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'videos');

DROP POLICY IF EXISTS "Authenticated update in videos bucket" ON storage.objects;
CREATE POLICY "Authenticated update in videos bucket"
  ON storage.objects FOR UPDATE
  TO authenticated
  USING (bucket_id = 'videos')
  WITH CHECK (bucket_id = 'videos');

DROP POLICY IF EXISTS "Authenticated delete in videos bucket" ON storage.objects;
CREATE POLICY "Authenticated delete in videos bucket"
