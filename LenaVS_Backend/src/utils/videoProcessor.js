...
const wrapTextForAss = (stanza, resolution) => {
  ...
  const fontSize = clamp(Math.round(normalizeNumber(stanza?.fontSize, 32)), 12, 220);
  ...
  const measureText = (line) => String(line || '').length * fontSize * widthFactor;
...
const buildSubtitleTags = (stanza, resolution) => {
  ...
  const tags = [
    `\\an${alignCode}`,
    `\\fn${fontFamily}`,
    `\\fs${clamp(Math.round(normalizeNumber(stanza?.fontSize, 32)), 12, 220)}`,
  ...
  if (transition === 'slide') {
    tags.push(`\\move(${anchorX},${anchorY + 30},${anchorX},${anchorY},0,${durationMs})`);
  } else {
