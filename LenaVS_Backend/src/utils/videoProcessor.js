...
const SUBTITLE_TIMELINE_EPSILON = 0.001;
...
export const createAssSubtitleFile = async (stanzas = [], outputPath, resolution = '720p') => {
  ...
  const dialogueLines = timelineEntries.flatMap((entry, index) => {
      ...
      const hasImmediateNextStanza = Boolean(
        nextEntry && nextEntry.displayStart <= entry.displayEnd + SUBTITLE_TIMELINE_EPSILON
      );
      const shouldApplyExitTransition = !hasImmediateNextStanza;

      if (!shouldApplyExitTransition || totalDuration <= 0.12) {
