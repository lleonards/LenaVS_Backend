...
export const generateOptimizedFinalVideo = async ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (backgroundType === 'video' && backgroundPath) {
      ...
      command
        .input(`color=c=${sanitizeHexColor(backgroundColor, '#000000')}:s=${width}x${height}:r=30:d=${safeDuration}`)
      ...
      const backgroundStage = [
        `[0:v]${buildVideoBackgroundFilter({ width, height })}`,
        ...fadeFilters,
        `scale=w='${width}*(${transformExpressions.scaleExpr})':h='${height}*(${transformExpressions.scaleExpr})':eval=frame`,
        'setsar=1[bgfx]',
      ].join(',');

      complexFilters.push(backgroundStage);
      complexFilters.push(
        `[2:v][bgfx]overlay=x='(W-w)/2':y='(H-h)/2+(${transformExpressions.yExpr})':eval=frame[bgcomposed]`
      );
    ...
    if (backgroundType === 'image' && backgroundPath) {
      ...
      const complexFilters = [];
      const backgroundStage = [
        `[0:v]${buildImageBackgroundFilter({ width, height })}`,
        ...fadeFilters,
        `scale=w='${width}*(${transformExpressions.scaleExpr})':h='${height}*(${transformExpressions.scaleExpr})':eval=frame`,
        'setsar=1[bgfx]',
      ].join(',');
