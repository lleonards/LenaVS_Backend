...
// O filtro "fade" do ffmpeg, fora da sua própria janela [st, st+d], força o alfa
// para 0 (tipo "in", antes do início) ou para 0 (tipo "out", depois do fim) em vez de
// deixar o frame passar inalterado. Como vários "fade" são encadeados em sequência
// (intro, outro e os fades de cada costura de loop), qualquer fade no meio da timeline
// acaba zerando o alfa do restante do vídeo, apagando o fundo inteiro. Adicionar
// "enable=between(...)" restringe cada fade à sua própria janela, deixando os frames
// fora dela intactos (com o alfa que os fades anteriores já haviam definido).
const buildFadeEnableClause = (start, duration) => {
  const safeStart = Math.max(0, normalizeNumber(start, 0));
  const safeEnd = safeStart + Math.max(0, normalizeNumber(duration, 0));
  return `enable='between(t\\,${formatExprNumber(safeStart)}\\,${formatExprNumber(safeEnd)})'`;
};
...
const buildFadeFilters = ({ totalDuration, mediaDuration, config }) => {
  ...
  if (normalizedConfig.introTransition === 'fade') {
    filters.push(`fade=t=in:st=0:d=${formatExprNumber(normalizedConfig.introDuration)}:alpha=1:${buildFadeEnableClause(0, normalizedConfig.introDuration)}`);
  }

  if (normalizedConfig.outroTransition === 'fade') {
    const start = Math.max(0, safeTotalDuration - normalizedConfig.outroDuration);
    filters.push(`fade=t=out:st=${formatExprNumber(start)}:d=${formatExprNumber(normalizedConfig.outroDuration)}:alpha=1:${buildFadeEnableClause(start, normalizedConfig.outroDuration)}`);
  }
  ...
  if (normalizedConfig.loopTransition === 'fade' && safeMediaDuration > 0 && safeMediaDuration < safeTotalDuration) {
    ...
    if (halfLoopDuration > 0.05) {
      ...
      for (
        ...
        filters.push(`fade=t=out:st=${formatExprNumber(fadeOutStart)}:d=${formatExprNumber(halfLoopDuration)}:alpha=1:${buildFadeEnableClause(fadeOutStart, halfLoopDuration)}`);
        filters.push(`fade=t=in:st=${formatExprNumber(boundary)}:d=${formatExprNumber(halfLoopDuration)}:alpha=1:${buildFadeEnableClause(boundary, halfLoopDuration)}`);
      }
    }
  }
