...
const buildBackgroundOverlayStage = ({
  ...
  return [
    ...
    'setsar=1[bgfx]',
    `[2:v][bgfx]overlay=x='(W-w)/2':y='(H-h)/2+(${transformExpressions.yExpr})':eval=frame:shortest=1:eof_action=repeat[bgcomposed]`,
  ];
};
...
export const generateOptimizedFinalVideo = async ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (backgroundType === 'video' && backgroundPath) {
      ...
      const complexFilters = buildBackgroundOverlayStage({
