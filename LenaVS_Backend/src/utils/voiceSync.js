...
/**
 * Tenta isolar vocal usando ORIGINAL e INSTRUMENTAL (cancelamento de fase):
 *   vocals ≈ original - instrumental
 *
 * Requisitos para funcionar bem:
 *  - instrumental precisa estar bem alinhado com o original (mesmo início/tempo)
 *
 * Saída: MP3 mono 16kHz (leve) para Whisper.
 */
function isolateVocalsFromOriginalAndInstrumental(originalPath, instrumentalPath) {
  return new Promise((resolve, reject) => {
    const output = path.join(os.tmpdir(), `lena_vocals_${Date.now()}.mp3`);

    // Padroniza ambos para mono/16k e subtrai via amix com peso negativo.
    // Observação: amix aceita weights negativos.
    ffmpeg()
      .input(originalPath)
      .input(instrumentalPath)
      .complexFilter([
        '[0:a]aformat=channel_layouts=mono,aresample=16000[a0]',
        '[1:a]aformat=channel_layouts=mono,aresample=16000[a1]',
        "[a0][a1]amix=inputs=2:weights='1 -1':normalize=0,volume=2.0,highpass=f=90,lowpass=f=9000,alimiter=limit=0.95[v]"
      ])
      .outputOptions(['-map [v]'])
      .audioChannels(1)
      .audioFrequency(16000)
...
async function transcribeWordsWithWhisper(localAudioPath) {
...
/**
 * Gera um trecho curto do áudio (para scoring/seleção) sem gastar Whisper no arquivo inteiro.
 */
...
function computeNWCoverage(stanzas, whisperWords, opts = {}) {
  const { maxLyricsWords = null } = opts;

  if (!whisperWords || whisperWords.length === 0) {
    return { coverage: 0, matched: 0, lyricsCount: 0, whisperCount: 0 };
  }
  ...
  const clipped = (Number.isFinite(maxLyricsWords) && maxLyricsWords > 0)
    ? lyricsFlat.slice(0, maxLyricsWords)
    : lyricsFlat;

  if (clipped.length === 0) {
    return { coverage: 0, matched: 0, lyricsCount: 0, whisperCount: whisperWords.length };
  }

  const lNorm = clipped.map(lw => lw.word);
  const alignment = needlemanWunsch(lNorm, wNorm);

  let matched = 0;
  for (const { lyricsIdx, whisperIdx } of alignment) {
    if (lyricsIdx !== null && whisperIdx !== null) matched++;
  }

  const coverage = matched / clipped.length;
  return {
    coverage,
    matched,
    lyricsCount: clipped.length,
    whisperCount: whisperWords.length
  };
}

/**
 * Escolhe automaticamente qual áudio (entre candidatos) “parece ter a letra”
 * usando um Whisper curto + score de cobertura via Needleman-Wunsch.
 *
 * Motivação:
 *  - usuário pode fazer upload invertido (instrumental como original)
 *  - usuário pode enviar dois áudios com voz diferentes
 *
 * Estratégia:
 *  1) pega um snippet (ex: a partir de 20s, 60s de duração)
 *  2) Whisper word timestamps (só para obter palavras)
 *  3) calcula coverage contra as primeiras N palavras da letra
 *  4) escolhe o maior coverage (empate: mais palavras)
 */
export async function chooseBestAudioCandidateByLyrics(audioUrls, stanzas, opts = {}) {
  ...
  if (candidates.length === 0) {
    throw new Error('audioUrls vazio');
  }
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      const words = await transcribeWordsWithWhisper(snip);
      const cov = computeNWCoverage(stanzas, words, { maxLyricsWords });

      // score: coverage pesa muito mais que quantidade de palavras
      const score = cov.coverage * 1000 + Math.min(cov.whisperCount, 300) * 0.5;
      ...
      results.push({
        ...
        matched: cov.matched,
        lyricsCount: cov.lyricsCount,
        whisperCount: cov.whisperCount
      });
    } catch (err) {
      results.push({
        index: i,
...
export async function syncStanzasWithWhisper(audioUrl, stanzas, opts = {}) {
  const { instrumentalUrl = null, forceOriginalOnly = false } = opts;
  const files = [];
  ...
  try {
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        // Se o usuário forneceu instrumental, tenta isolar vocal para melhorar a transcrição
        let whisperInputPath = rawPath;

        if (!forceOriginalOnly && instrumentalUrl) {
          try {
            const instPath = await downloadToTemp(instrumentalUrl);
            files.push(instPath);

            const vocalsPath = await isolateVocalsFromOriginalAndInstrumental(rawPath, instPath);
            files.push(vocalsPath);

            whisperInputPath = vocalsPath;
            console.log('[voiceSync] 🎤 Usando vocal isolado (original - instrumental) para Whisper');
          } catch (isoErr) {
            console.warn('[voiceSync] Falha ao isolar vocal, usando áudio original:', isoErr.message);
          }
        }

        const words = await transcribeWordsWithWhisper(whisperInputPath);
