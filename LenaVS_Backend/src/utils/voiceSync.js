...
async function createVocalGuideTrack(inputPath) {
  return new Promise((resolve, reject) => {
    const output = path.join(os.tmpdir(), `lena_vocalguide_${Date.now()}.wav`);
...
function buildTranscriptAnalysis(words, stanzas, totalDur) {
  const safeWords = Array.isArray(words)
    ? words.filter((item) => normalizeWord(item?.word).length > 0)
    : [];

  const normalized = safeWords.map((item) => normalizeWord(item.word)).filter(Boolean);
  const uniqueWords = new Set(normalized).size;
  const wordDensity = safeWords.length / Math.max(1, Number(totalDur) || 1);

  const rawMatches = safeWords.length > 0 && Array.isArray(stanzas) && stanzas.length > 0
    ? fuzzyMatchStanzas(stanzas, safeWords)
    : stanzas.map(() => null);

  const matchedCount = rawMatches.filter((match) => isUsableRawMatch(match)).length;
  const avgScore = rawMatches.length
    ? rawMatches.reduce((acc, match) => acc + Number(match?.score ?? match?.rawScore ?? 0), 0) / rawMatches.length
    : 0;
  const coverage = matchedCount / Math.max(1, stanzas.length || 1);

  const instrumentalLikely = safeWords.length === 0 || (
    coverage < 0.12
    && avgScore < 0.20
    && uniqueWords < 12
    && wordDensity < 1.15
  ...
  const score = coverage * 1000
    + matchedCount * 24
    + avgScore * 140
    + uniqueWords * 0.8
    + safeWords.length * 0.1
    - (instrumentalLikely ? 180 : 0);
...
async function pickBestSyncTranscriptSource(rawPath, stanzas, totalDur) {
  const tempFiles = [];
  const candidates = [{ kind: 'original', path: rawPath }];

  try {
    const vocalGuidePath = await createVocalGuideTrack(rawPath);
    tempFiles.push(vocalGuidePath);
    candidates.unshift({ kind: 'vocal-guide', path: vocalGuidePath });
  } catch (error) {
    console.warn('[voiceSync] Falha ao criar guia vocal:', error.message);
  }
  ...
  for (const candidate of candidates) {
    try {
      ...
      const analysis = buildTranscriptAnalysis(words, stanzas, totalDur);
      ...
      analyzedCandidates.push({
        ...
        words: [],
        engine: 'error',
        analysis: buildTranscriptAnalysis([], stanzas, totalDur),
        error: error.message
      });
    }
  }

  analyzedCandidates.sort((a, b) => b.analysis.score - a.analysis.score);
  const best = analyzedCandidates[0] || {
    kind: 'original',
    path: rawPath,
    words: [],
    engine: 'none',
  ...
  const instrumentalOnly = Boolean(best.analysis.instrumentalLikely && best.analysis.matchedCount === 0);

  return {
    ...best,
    instrumentalOnly,
    tempFiles,
    candidateReports: analyzedCandidates.map((candidate) => ({
      kind: candidate.kind,
      engine: candidate.engine,
      wordCount: candidate.analysis.wordCount,
      uniqueWords: candidate.analysis.uniqueWords,
      matchedCount: candidate.analysis.matchedCount,
      coverage: candidate.analysis.coverage,
      avgScore: candidate.analysis.avgScore,
      score: candidate.analysis.score,
      instrumentalLikely: candidate.analysis.instrumentalLikely,
      error: candidate.error || null
    }))
  };
}

function stanzaIdentity(stanza) {
  return extractWordsAllowShort(String(stanza?.text || '')).slice(0, 12).join(' ');
}

function findPreviousUsableMatch(matches, index) {
  for (let i = index - 1; i >= 0; i--) {
    if (isUsableRawMatch(matches[i])) return { index: i, match: matches[i] };
  }
  return null;
}

function findNextUsableMatch(matches, index) {
...
function repairOverlappingAndRepeatedMatches(rawMatches, stanzas, whisperWords) {
  if (!Array.isArray(rawMatches) || !Array.isArray(whisperWords) || whisperWords.length === 0) {
    return rawMatches;
  }

  const repaired = [...rawMatches];

  for (let i = 0; i < repaired.length; i++) {
    const current = repaired[i];
    if (!current) continue;

    const prevInfo = findPreviousUsableMatch(repaired, i);
    if (!prevInfo) continue;

    const prev = prevInfo.match;
    const hasOverlap = Number(current.startIdx ?? 0) <= Number(prev.endIdx ?? -1);
    const repeatedTextAssignedTwice = stanzaIdentity(stanzas[i])
      && stanzaIdentity(stanzas[prevInfo.index])
      && stanzaIdentity(stanzas[i]) === stanzaIdentity(stanzas[prevInfo.index])
      && Math.abs(Number(current.startTime ?? 0) - Number(prev.startTime ?? 0)) < 1.2;

    if (!hasOverlap && !repeatedTextAssignedTwice) continue;

    const searchStart = Math.min(whisperWords.length - 1, Math.max(0, Number(prev.endIdx ?? 0) + 1));
    const nextInfo = findNextUsableMatch(repaired, i + 1);
    const searchEnd = nextInfo
      ? Math.max(searchStart + 2, Number(nextInfo.match.startIdx ?? whisperWords.length - 1) - 1)
    ...
    const candidate = matchSingleStanzaInWindow(stanzas[i], whisperWords, searchStart, searchEnd);
    if (candidate && Number(candidate.startIdx ?? 0) > Number(prev.endIdx ?? -1)) {
...
function computeSyncConfidence(rawMatch, filledMatch) {
  const baseScore = Number(rawMatch?.score ?? rawMatch?.rawScore ?? 0);
  const startTime = Number(filledMatch?.startTime ?? rawMatch?.startTime ?? 0);
  const endTime = Number(filledMatch?.endTime ?? rawMatch?.endTime ?? startTime);
  const duration = Math.max(0, endTime - startTime);

  let confidence = Math.max(0, baseScore) * 1.45;
  confidence += Math.min(duration / 1.2, 1) * 0.12;

  if (!isUsableRawMatch(rawMatch)) confidence *= 0.55;
...
function isUsableRawMatch(match) {
  if (!match) return false;
  const start = Number(match.startTime);
  const end = Number(match.endTime);
  const score = Number(match.score ?? match.rawScore ?? 0);
...
function buildSyncReport(rawMatches, filledMatches, stanzas, attemptsByStanza, maxAttempts, meta = {}) {
  const stanzaReports = stanzas.map((stanza, idx) => {
    const raw = rawMatches[idx];
    const filled = filledMatches[idx];
    const synced = isUsableRawMatch(raw);
    const startTime = Number(filled?.startTime ?? raw?.startTime ?? 0);
    const endTime = Number(filled?.endTime ?? raw?.endTime ?? startTime);
    const duration = Math.max(0, endTime - startTime);
    const confidence = computeSyncConfidence(raw, filled);
    const needsReview = !synced || confidence < 0.40;

    return {
      id: stanza?.id ?? idx,
      stanzaIndex: idx,
      textPreview: String(stanza?.text || '').replace(/\s+/g, ' ').trim().slice(0, 90),
      attempts: attemptsByStanza[idx] || 1,
      synced,
      usedFallback: !synced,
      needsReview,
      confidence,
      score: Number(raw?.score ?? raw?.rawScore ?? 0),
      startTime,
      endTime,
      duration,
      status: synced
        ? ((attemptsByStanza[idx] || 1) > 1 ? 'retried-success' : 'synced')
        : 'fallback-interpolated'
    };
  });

  return {
    totalStanzas: stanzaReports.length,
    syncedCount: stanzaReports.filter((item) => item.synced).length,
    fallbackCount: stanzaReports.filter((item) => item.usedFallback).length,
    retriedCount: stanzaReports.filter((item) => item.attempts > 1).length,
    needsReviewCount: stanzaReports.filter((item) => item.needsReview).length,
    maxAttempts,
    instrumentalOnly: Boolean(meta.instrumentalOnly),
    selectedSourceKind: meta.selectedSourceKind || 'original',
    engine: meta.engine || 'none',
    candidateReports: Array.isArray(meta.candidateReports) ? meta.candidateReports : [],
    stanzas: stanzaReports
  };
}

function buildRetryWindows(idx, rawMatches, stanzas, totalWords) {
  const prev = (() => {
    for (let i = idx - 1; i >= 0; i--) {
      if (isUsableRawMatch(rawMatches[i])) return rawMatches[i];
    }
    return null;
  })();

  const next = (() => {
    for (let i = idx + 1; i < rawMatches.length; i++) {
      if (isUsableRawMatch(rawMatches[i])) return rawMatches[i];
    }
    return null;
  })();

  const estimated = estimateWindowSizeFromStanza(stanzas[idx]);
  const baseBuffer = Math.max(10, Math.round(estimated * 0.75));
  const windows = [];

  if (prev && next && next.startIdx > prev.endIdx) {
    windows.push({
      start: Math.max(0, prev.endIdx + 1),
      end: Math.min(totalWords - 1, next.startIdx - 1),
      label: 'between-neighbors'
    });
  }

  if (prev) {
    windows.push({
      start: Math.max(0, prev.endIdx + 1 - baseBuffer),
      end: Math.min(totalWords - 1, prev.endIdx + estimated + baseBuffer),
  ...
  if (next) {
    windows.push({
      start: Math.max(0, next.startIdx - estimated - baseBuffer),
      end: Math.min(totalWords - 1, next.startIdx + baseBuffer),
      label: 'before-next'
    });
  }
  ...
  if (prev && next) {
    windows.push({
      start: Math.max(0, prev.endIdx + 1 - baseBuffer * 2),
      end: Math.min(totalWords - 1, next.startIdx - 1 + baseBuffer * 2),
      label: 'expanded-neighborhood'
    });
  }

  windows.push({ start: 0, end: totalWords - 1, label: 'full-audio' });

  const deduped = [];
  const seen = new Set();
  for (const win of windows) {
    const start = Math.max(0, Number(win.start) || 0);
    const end = Math.min(totalWords - 1, Number(win.end) || 0);
    if (end - start < 2) continue;
    const key = `${start}:${end}`;
    if (seen.has(key)) continue;
    seen.add(key);
    deduped.push({ ...win, start, end });
  }
  return deduped;
}

function matchSingleStanzaInWindow(stanza, whisperWords, globalStart, globalEnd) {
  if (!Array.isArray(whisperWords) || whisperWords.length === 0) return null;
  const safeStart = Math.max(0, Number(globalStart) || 0);
  const safeEnd = Math.min(whisperWords.length - 1, Number(globalEnd) || 0);
  if (safeEnd - safeStart < 2) return null;

  const slicedWords = whisperWords.slice(safeStart, safeEnd + 1);
  const candidate = fuzzyMatchStanzas([stanza], slicedWords)[0];
  if (!candidate) return null;

  return {
    ...candidate,
    text: String(stanza?.text || ''),
    startIdx: (candidate.startIdx ?? 0) + safeStart,
    endIdx: (candidate.endIdx ?? 0) + safeStart
  };
}
...
function isBetterMatch(candidate, current) {
  ...
  const candidateUsable = isUsableRawMatch(candidate);
  const currentUsable = isUsableRawMatch(current);
  const candidateScore = Number(candidate.score ?? candidate.rawScore ?? 0);
  const currentScore = Number(current.score ?? current.rawScore ?? 0);

  if (candidateUsable && !currentUsable) return true;
  if (candidateUsable === currentUsable && candidateScore > currentScore + 0.02) return true;

  const candidateDuration = Number(candidate.endTime ?? 0) - Number(candidate.startTime ?? 0);
  const currentDuration = Number(current.endTime ?? 0) - Number(current.startTime ?? 0);
  if (candidateUsable === currentUsable && Math.abs(candidateScore - currentScore) <= 0.02) {
    if (candidateDuration > 0.35 && currentDuration <= 0.35) return true;
  }

  return false;
}

export async function syncStanzasWithWhisperRetrying(audioUrl, stanzas, opts = {}) {
  const files = [];
  const cleanup = () => {
  ...
  const maxAttempts = Math.max(1, Math.min(5, Number(opts.maxAttempts) || 5));
  ...
  try {
    ...
    try {
      const transcriptChoice = await pickBestSyncTranscriptSource(rawPath, stanzas, totalDur);
      files.push(...(transcriptChoice.tempFiles || []));

      const sourceMeta = {
        instrumentalOnly: transcriptChoice.instrumentalOnly,
        selectedSourceKind: transcriptChoice.kind,
        engine: transcriptChoice.engine,
        candidateReports: transcriptChoice.candidateReports
      };

      if (transcriptChoice.instrumentalOnly) {
        const emptyMatches = stanzas.map(() => null);
        return {
          segments: [],
          report: buildSyncReport(emptyMatches, emptyMatches, stanzas, stanzas.map(() => 1), maxAttempts, sourceMeta),
          method: 'instrumental-detected',
          instrumentalOnly: true
        };
      }

      const { words, engine } = transcriptChoice;
      if (words.length > 0) {
        console.log(`[voiceSync] ✅ ${engine}: ${words.length} palavras com timestamps (${transcriptChoice.kind})`);

        const attemptsByStanza = stanzas.map(() => 1);
        const initialMatches = Array.isArray(transcriptChoice.analysis?.rawMatches)
          && transcriptChoice.analysis.rawMatches.length === stanzas.length
          ? transcriptChoice.analysis.rawMatches
          : fuzzyMatchStanzas(stanzas, words);

        let rawMatches = repairOverlappingAndRepeatedMatches(initialMatches, stanzas, words);

        for (let pass = 2; pass <= maxAttempts; pass++) {
          const unsyncedIndexes = rawMatches
            .map((match, idx) => (!isUsableRawMatch(match) ? idx : -1))
            .filter((idx) => idx >= 0);

          if (unsyncedIndexes.length === 0) break;

          let improvedInPass = false;

          for (const idx of unsyncedIndexes) {
            let bestMatch = rawMatches[idx];
            const retryWindows = buildRetryWindows(idx, rawMatches, stanzas, words.length);

            for (const win of retryWindows) {
              if (attemptsByStanza[idx] >= maxAttempts) break;
              attemptsByStanza[idx] += 1;

              const candidate = matchSingleStanzaInWindow(stanzas[idx], words, win.start, win.end);
              if (isBetterMatch(candidate, bestMatch)) {
                bestMatch = candidate;
                improvedInPass = true;
              }
              if (isUsableRawMatch(bestMatch)) break;
            }

            rawMatches[idx] = bestMatch;
          }

          rawMatches = repairOverlappingAndRepeatedMatches(rawMatches, stanzas, words);

          if (!improvedInPass) {
            const remainingUnsynced = rawMatches.some((match) => !isUsableRawMatch(match));
            if (!remainingUnsynced) break;
          }
        }

        const filledMatches = fillNullMatches(rawMatches, stanzas, totalDur);
        return {
          segments: toLineLevelFormat(filledMatches, stanzas, totalDur),
          report: buildSyncReport(rawMatches, filledMatches, stanzas, attemptsByStanza, maxAttempts, sourceMeta),
          method: 'whisper-vocal-guide-verified',
    ...
    if (silResult) {
      const filledMatches = silResult.map((item) => ({
        text: item?.text || '',
      ...
      return {
        segments: toLineLevelFormat(silResult, stanzas, totalDur),
        report: buildSyncReport(stanzas.map(() => null), filledMatches, stanzas, stanzas.map(() => 1), maxAttempts, {
          selectedSourceKind: 'fallback',
          engine: 'silence-detect'
        }),
        method: 'silence-detect-fallback',
        instrumentalOnly: false
      };
    }

    const propResult = fallbackProportional(stanzas, totalDur || 180);
    const filledMatches = propResult.map((item) => ({
      text: item?.text || '',
      startTime: item?.startTime ?? 0,
      endTime: item?.endTime ?? 0,
      score: 0
    ...
    return {
      segments: toLineLevelFormat(propResult, stanzas, totalDur || 180),
      report: buildSyncReport(stanzas.map(() => null), filledMatches, stanzas, stanzas.map(() => 1), maxAttempts, {
        selectedSourceKind: 'fallback',
        engine: 'proportional'
      }),
...
export async function syncStanzasWithWhisper(audioUrl, stanzas, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      if (transcriptChoice.instrumentalOnly) {
        console.log('[voiceSync] Áudio classificado como instrumental — sem estrofes visíveis');
        return [];
      }
      ...
      if (words.length > 0) {
        console.log(`[voiceSync] ✅ ${engine}: ${words.length} palavras com timestamps (${transcriptChoice.kind})`);
        const rawMatches = repairOverlappingAndRepeatedMatches(
          Array.isArray(transcriptChoice.analysis?.rawMatches) && transcriptChoice.analysis.rawMatches.length === stanzas.length
            ? transcriptChoice.analysis.rawMatches
            : fuzzyMatchStanzas(stanzas, words),
          stanzas,
          words
        ...
        return toLineLevelFormat(filled, stanzas, totalDur);
      }
