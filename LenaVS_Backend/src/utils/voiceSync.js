 ...
 * Importante: não altera whisperWords (timestamps), só as listas usadas no score.
 */
...
function blockSimilarity(aWords, bWords) {
  ...
  const scoreOriginal = 0.6 * jaccard1 + 0.4 * jaccard2;

  // ── Penalidade por tamanho (mantém matches corretos e evita janelas gigantes) ──
  // Recomendação: scoreFinal = scoreOriginal * (min(lenA,lenB)/max(lenA,lenB))
  const lenA = aWords.length;
  const lenB = bWords.length;
  const lengthPenalty = Math.min(lenA, lenB) / Math.max(lenA, lenB);

  return scoreOriginal * lengthPenalty;
}
...
function fuzzyMatchStanzas(stanzas, whisperWords) {
  ...
  let lastEndTime = 0; // garante ordem temporal — nunca volta no tempo

  for (const stanza of stanzas) {
    const stanzaText     = String(stanza.text || '');
    const stanzaWordsRaw = extractWordsAllowShort(stanzaText);

    // Redução de ruído: remove repetições consecutivas (canto sustentado)
    const stanzaWords = removeConsecutiveDuplicates(stanzaWordsRaw);
    const stanzaSet   = new Set(stanzaWords);
    ...
    // Mantém a heurística de tamanho de janela baseada no "raw" (mais estável)
    const winSize = Math.max(MIN_WINDOW, Math.round(stanzaWordsRaw.length * WINDOW_RATIO));

    let bestScore = -1;     // score ajustado (com penalidades suaves)
    let bestRawScore = -1;  // score base do blockSimilarity (já inclui lengthPenalty)
    let bestStart = -1;
    let bestEnd   = -1;

    // Desliza a janela a partir do cursor
    for (let i = cursor; i <= total - Math.max(1, stanzaWordsRaw.length - 2); i++) {
      ...
      for (let w = winMin; w <= winMax; w++) {
        ...
        // Redução de ruído também no lado do Whisper (só p/ pontuação)
        const windowWordsRaw = wNorm.slice(i, i + w);
        const windowWords    = removeConsecutiveDuplicates(windowWordsRaw);

        // Score base (já tem penalidade por tamanho dentro de blockSimilarity)
        const rawScore = blockSimilarity(stanzaWords, windowWords);

        // ── Penalidade baseada em duração (INTELIGENTE e gradual) ─────────────
        const startT = whisperWords[i]?.start ?? 0;
        if (startT < lastEndTime) continue; // respeita ordem temporal
        const endT   = whisperWords[i + w - 1]?.end ?? whisperWords[i + w - 1]?.start ?? startT;
        ...
        const durationRatio = expectedDuration / duration;
        const durationPenalty = Math.min(1, durationRatio);

        let score = rawScore * durationPenalty;
        // ── Detectar "mistura" de estrofes (muitas palavras extras) ───────────
        let matchingWords = 0;
        for (const ww of windowWords) {
          if (stanzaSet.has(ww)) matchingWords++;
        }

        const extraWordsRatio = windowWords.length > 0
          ? (windowWords.length - matchingWords) / windowWords.length
          : 1;
    ...
    if (bestScore >= MIN_SIMILARITY && bestStart >= 0) {
      ...
      // Atualiza barreira temporal — impede reutilização de trechos passados
      lastEndTime = whisperWords[bestEnd]?.end ?? whisperWords[bestEnd]?.start ?? 0;
      lastEndTime += 0.05; // pequena margem anti-colisão

      // Avança cursor (mais seguro quando há sobreposição entre estrofes)
      cursor = Math.max(
        bestStart + Math.floor((bestEnd - bestStart) * 0.6),
        bestEnd - 2
      );
      // Safety: garante progresso mínimo (evita loop em casos degenerados)
