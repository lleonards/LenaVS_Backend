...
function buildSyncReport(rawMatches, filledMatches, stanzas, attemptsByStanza, maxAttempts) {
...
function estimateWindowSizeFromStanza(stanza) {
  const words = extractWordsAllowShort(stanza?.text || '');
  return Math.max(8, Math.round(words.length * 4));
}
  ...
  try {
    ...
    try {
      ...
      if (words.length > 0) {
        ...
        const segments = toLineLevelFormat(filledMatches, stanzas, totalDur);
        const report = buildSyncReport(rawMatches, filledMatches, stanzas, attemptsByStanza, maxAttempts);

        return {
          segments,
          report,
          method: 'whisper-verified-retries'
        };
      }
