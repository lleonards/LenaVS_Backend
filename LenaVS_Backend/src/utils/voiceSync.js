...
export async function syncStanzasWithWhisper(audioUrl, stanzas, opts = {}) {
  ...
  try {
    ...
    // ── Word timestamps + vocal guide + análise de confiança ─────────────────
    try {
      const ts = await getBestWordTimestampsForSync(rawPath, stanzas, files);
      ...
      if (words.length > 0) {
        ...
        const analysis = buildSyncAnalysis(stanzas, filled, words, totalDur, {
          source,
          engine,
          mode: 'original+vocal-guide',
        ...
        const enriched = appendAnalysisToMatches(filled, analysis);
        const result = toLineLevelFormat(enriched, stanzas, totalDur);

        console.log('[voiceSync] ✅ Sincronização automática validada por análise de confiança');
        return {
          segments: result,
          analysis
        };
