...
const updateStripeSubscriptionState = async ({
  customerId = null,
  email = null,
  subscriptionId = null,
  ...
  if (subscriptionId) {
    payload.stripe_subscription_id = String(subscriptionId);
  }
...
const markUserForCancellationSurvey = async ({ customerId = null, subscriptionId = null, email = null } = {}) => {
  if (!customerId && !subscriptionId && !email) return;

  const patch = { cancellation_survey_pending: true, updated_at: new Date().toISOString() };
  let query = supabase.from('users').update(patch).select('id');
  ...
  const { error } = await query.maybeSingle();
  if (error) {
    console.warn('Não foi possível marcar usuário para pesquisa de cancelamento:', error.message);
  }
};
  ...
  try {
    ...
    if (provider === PAGARME_PROVIDER) {
      ...
      return res.json({
        ...
        openMode: 'new_tab',
        statusUrl: '/payment/pending?provider=pagarme',
        countryGroup: checkoutContext.countryGroup,
        currency: 'BRL',
        currencySource: checkoutContext.source,
      });
    }
    ...
    if (!priceId) {
      return res.status(500).json({
        error: `Price ID do Stripe não configurado para ${checkoutContext.currency.toUpperCase()}`,
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        email: profile.email,
        plan: 'unlimited',
        country_group: checkoutContext.countryGroup,
        checkout_currency: checkoutContext.currency.toUpperCase(),
        currency_source: checkoutContext.source,
      },
      subscription_data: {
        metadata: {
          user_id: profile.id,
    ...
    return res.json({
      ...
      openMode: 'redirect',
      countryGroup: checkoutContext.countryGroup,
      currency: checkoutContext.currency.toUpperCase(),
      currencySource: checkoutContext.source,
    });
  } catch (err) {
...
export const handleStripeWebhook = async (req, res) => {
  ...
  try {
    if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
      ...
      const isSubscriptionCheckout = String(session.mode || '').toLowerCase() === 'subscription';
      const isPaidCheckout = ['paid', 'no_payment_required'].includes(String(session.payment_status || '').toLowerCase())
        || String(session.status || '').toLowerCase() === 'complete';

      if (!isSubscriptionCheckout && isPaidCheckout && !alreadyGrantedAccess(transaction)) {
        await applyUnlimitedAccessToUser({
          userId,
          email: customerEmail,
          provider: STRIPE_PROVIDER,
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      await markUserForCancellationSurvey({ customerId, subscriptionId, email: customerEmail });
    }
