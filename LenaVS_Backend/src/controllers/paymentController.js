...
const isHashRouterEnabled = () => ['1', 'true', 'yes', 'on'].includes(String(process.env.FRONTEND_USE_HASH_ROUTER || '').trim().toLowerCase());

const buildFrontendAppUrl = (route = '/') => {
  const frontendUrl = getFrontendUrl();
  const normalizedRoute = String(route || '/').startsWith('/') ? String(route || '/') : `/${String(route || '/')}`;

  if (frontendUrl.includes('/#')) {
    return `${frontendUrl}${normalizedRoute}`;
  }

  return isHashRouterEnabled()
    ? `${frontendUrl}/#${normalizedRoute}`
    : `${frontendUrl}${normalizedRoute}`;
};
...
const getReturnUrls = (provider) => ({
  success: buildFrontendAppUrl(`/payment/success?provider=${provider}`),
  pending: buildFrontendAppUrl(`/payment/pending?provider=${provider}`),
  failure: buildFrontendAppUrl(`/payment/failure?provider=${provider}`),
  cancel: buildFrontendAppUrl(`/upgrade?canceled=1&provider=${provider}`),
});
...
const createMercadoPagoPreference = async ({ profile, currency = 'brl', paymentMethod = 'pix_boleto' }) => {
  const accessToken = getMercadoPagoAccessToken();
  ...
  const normalizedPaymentMethod = String(paymentMethod || 'pix_boleto').toLowerCase();
...
export const createPaymentSession = async (req, res) => {
  try {
    ...
    const paymentMethod = String(req.body?.paymentMethod || 'pix_boleto').toLowerCase();
