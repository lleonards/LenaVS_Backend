...
// "price" permanece somente como valor histórico permitido no banco.
// Novos envios são rejeitados para acompanhar as opções atuais do frontend.
const LEGACY_CANCELLATION_REASONS = new Set(['price']);
...
export const submitCancellationFeedback = async (req, res) => {
  try {
    ...
    if (!ACCOUNT_DELETION_REASONS.has(reason) || LEGACY_CANCELLATION_REASONS.has(reason)) {
      return res.status(400).json({ error: 'Motivo de cancelamento inválido.' });
    }
    ...
    await saveAccountDeletionFeedback({
      ...
      profile: currentProfile,
      deletionSummary: {
        privateProjectsDeleted: 0,
        publicProjectsDetached: 0,
      },
      stripeCancellation: {
        attempted: false,
        canceled: false,
    ...
    const databaseMissingMessage = String(error?.message || '').toLowerCase().includes('account_deletion_feedback')
      ? 'Aplique a migration de feedback no Supabase antes de usar este recurso.'
      : null;

    return res.status(500).json({
      error: databaseMissingMessage || error?.message || 'Não foi possível salvar o feedback.',
    });
  }
};
