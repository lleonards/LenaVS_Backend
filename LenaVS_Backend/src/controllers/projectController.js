...
const cloneProjectMediaFilesForUser = async (mediaFiles = {}, targetUserId) => {
  ...
  if (originalHadAnyAudio && !clonedHasAnyAudio) {
    const unavailableAudioError = new Error(
      'O áudio deste projeto público não pôde ser copiado. Reenvie o áudio no projeto original ou publique novamente antes de editar esta cópia.'
    );
    unavailableAudioError.status = 409;
    unavailableAudioError.details = issues;
    throw unavailableAudioError;
  }

  if (issues.length) {
    console.warn('Alguns arquivos de mídia do projeto não puderam ser copiados:', issues.join(' | '));
  }
