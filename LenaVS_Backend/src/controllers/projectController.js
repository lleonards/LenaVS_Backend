...
const AVAILABLE_PROJECT_RESOLUTIONS = new Set(['360p', '480p', '720p']);

const normalizeProjectResolution = (value) => {
  const normalized = String(value || DEFAULT_RESOLUTION).trim();
  return AVAILABLE_PROJECT_RESOLUTIONS.has(normalized) ? normalized : DEFAULT_RESOLUTION;
};
...
const cloneProjectMediaFilesForUser = async (mediaFiles = {}, targetUserId) => {
  ...
  const originalHadAnyAudio = Boolean(mediaFiles?.musicaOriginal || mediaFiles?.musicaInstrumental);
  const clonedHasAnyAudio = Boolean(clonedMediaFiles.musicaOriginal || clonedMediaFiles.musicaInstrumental);

  if (originalHadAnyAudio && !clonedHasAnyAudio) {
    clonedMediaFiles.musicaOriginal = mediaFiles?.musicaOriginal || clonedMediaFiles.musicaOriginal;
    clonedMediaFiles.musicaInstrumental = mediaFiles?.musicaInstrumental || clonedMediaFiles.musicaInstrumental;
    issues.push('Áudio reutilizado pelo link público original para evitar falha ao abrir o projeto na biblioteca.');
  }
...
export const createProject = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      resolution: normalizeProjectResolution(resolution),
...
export const updateProject = async (req, res) => {
  try {
    ...
    if (typeof resolution !== 'undefined') {
      updateData.resolution = normalizeProjectResolution(resolution);
    }
...
export const forkProject = async (req, res) => {
  try {
    ...
    const clonedMediaFiles = await cloneProjectMediaFilesForUser(normalizedOriginalConfig.mediaFiles || {}, userId);
    const forkedConfig = {
