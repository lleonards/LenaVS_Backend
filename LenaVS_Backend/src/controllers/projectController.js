...
const restrictMediaMetadataToLockedAudioType = (mediaMetadata = {}, lockedAudioType = null) => {
  const restricted = {
    musicaOriginal: null,
    musicaInstrumental: null,
    video: mediaMetadata?.video || null,
    imagem: mediaMetadata?.imagem || null,
  };

  if (lockedAudioType === 'instrumental') {
    restricted.musicaInstrumental = mediaMetadata?.musicaInstrumental || null;
    return restricted;
  }

  if (lockedAudioType === 'original') {
    restricted.musicaOriginal = mediaMetadata?.musicaOriginal || null;
    return restricted;
  }

  return {
    ...restricted,
    musicaOriginal: mediaMetadata?.musicaOriginal || null,
    musicaInstrumental: mediaMetadata?.musicaInstrumental || null,
  };
};
  ...
  if (ownersError) {
    ...
    (owners || []).forEach((owner) => {
      ownerMap[owner.id] = {
        ...
        owner_name: normalizeDisplayName(owner?.display_name) || ownerMap[owner.id]?.owner_name || UNKNOWN_OWNER_NAME,
      };
    });
  }
...
export const forkProject = async (req, res) => {
  try {
    ...
    const forkedConfig = {
      ...
      mediaMetadata: restrictMediaMetadataToLockedAudioType(normalizedOriginalConfig.mediaMetadata || {}, lockedAudioType),
    };
