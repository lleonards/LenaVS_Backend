...
// Arquivos de trabalho nunca devem ser expostos diretamente pelo Storage.
// O bucket público é usado somente depois que um projeto é publicado.
export const STORAGE_BUCKET = process.env.SUPABASE_STORAGE_BUCKET || 'Videos';
export const PUBLIC_STORAGE_BUCKET = process.env.SUPABASE_PUBLIC_STORAGE_KEY || 'VIDEOS_PUBLICOS';
export const PRIVATE_SIGNED_URL_EXPIRES_IN = 60 * 60;
...
export const getStoragePublicUrl = (storagePath, bucket = STORAGE_BUCKET) => {
...
export const createSignedStorageUrl = async (
  ...
  expiresIn = PRIVATE_SIGNED_URL_EXPIRES_IN
) => {
  if (!storagePath) return null;

  if (bucket === PUBLIC_STORAGE_BUCKET) {
    return getStoragePublicUrl(storagePath, bucket);
  }
...
export const getStorageAccessUrl = async (storagePath, {
  bucket = STORAGE_BUCKET,
  expiresIn = PRIVATE_SIGNED_URL_EXPIRES_IN,
} = {}) => createSignedStorageUrl(storagePath, bucket, expiresIn);

export const extractStorageReferenceFromUrl = (sourceValue) => {
...
export const resolveStorageReference = (sourceValue, metadata = {}) => {
  const metadataPath = String(metadata?.storagePath || '').trim();
  const metadataBucket = String(metadata?.bucket || '').trim();

  if (metadataPath) {
    return {
      bucket: metadataBucket || STORAGE_BUCKET,
      storagePath: metadataPath.replace(/^\/+/, ''),
    };
  }

  const fromUrl = extractStorageReferenceFromUrl(sourceValue);
  if (fromUrl) return fromUrl;

  const rawValue = String(sourceValue || '').trim();
  if (rawValue && !isHttpUrl(rawValue) && !rawValue.startsWith('/uploads/')) {
    return {
      bucket: metadataBucket || STORAGE_BUCKET,
      storagePath: rawValue.replace(/^\/+/, ''),
    };
  }
...
export const uploadLocalFileToStorage = async ({
  ...
  const accessUrl = await getStorageAccessUrl(storagePath, { bucket });
  if (!accessUrl) {
    throw new Error('Não foi possível gerar a URL de acesso do arquivo enviado.');
  }

  return {
    bucket,
    storagePath,
    // Mantemos o nome publicUrl para compatibilidade com o frontend legado.
    // Em Videos, o valor é uma URL assinada temporária, nunca uma URL pública.
    publicUrl: accessUrl,
    accessUrl,
  };
};
...
export const buildSourceUrlCandidates = (sourceValue) => {
  ...
  if (
    ...
    const storagePublicUrl = getStoragePublicUrl(normalizedStoragePath, STORAGE_BUCKET);
    if (storagePublicUrl) {
...
export const downloadSourceValueToTempFile = async (sourceValue, {
  ...
  const candidateUrls = [];
  const storageReference = resolveStorageReference(sourceValue, metadata);

  if (storageReference) {
    try {
      const accessUrl = await getStorageAccessUrl(storageReference.storagePath, {
        bucket: storageReference.bucket || bucket,
      });
      if (accessUrl) candidateUrls.push(accessUrl);
    } catch (error) {
      // Tenta a referência HTTP abaixo para preservar compatibilidade com
      // arquivos legados que já estavam armazenados como URL.
      console.warn('Não foi possível gerar URL de acesso do Storage:', error.message);
    }
  }

  candidateUrls.push(...buildSourceUrlCandidates(sourceValue));
  let lastError = null;
...
export const publishMediaReference = async (sourceValue, sourceMetadata = {}, {
  userId,
  projectId,
  fieldName = 'media',
} = {}) => {
  ...
  const fallbackName = `${sanitizeStorageSegment(fieldName)}${inferExtension({
    originalName: sourceMetadata?.originalName || '',
    mimeType: sourceMetadata?.mimeType || '',
    fallback: getDefaultExtensionForStorageField(fieldName),
  })}`;
  const tempPath = await downloadSourceValueToTempFile(rawValue, {
    prefix: `publish-${fieldName}`,
    fallbackName,
    mimeType: sourceMetadata?.mimeType || '',
    folder: 'publish',
    metadata: sourceMetadata,
  ...
  const storagePath = buildStorageObjectPath({
    category: `published/${projectId || 'project'}`,
    userId,
    prefix: fieldName,
    originalName: fallbackName,
    mimeType: sourceMetadata?.mimeType || '',
    fallbackExtension: getDefaultExtensionForStorageField(fieldName),
  });
  ...
  try {
    ...
    return {
      ...
      publicUrl: getStoragePublicUrl(uploaded.storagePath, PUBLIC_STORAGE_BUCKET),
      accessUrl: getStoragePublicUrl(uploaded.storagePath, PUBLIC_STORAGE_BUCKET),
    };
  } finally {
    await removeLocalFileSilently(tempPath);
  }
...
const getDefaultExtensionForStorageField = (fieldName) => {
  if (fieldName === 'video') return '.mp4';
  if (fieldName === 'imagem') return '.jpg';
  return '.mp3';
};
