...
-- - exclusão de conta com preservação de feedback
-- - manutenção de projetos públicos na Biblioteca da Comunidade
-- - remoção de projetos privados na exclusão de conta
...
CREATE TABLE IF NOT EXISTS public.projects (
  ...
  CONSTRAINT projects_private_requires_owner_check CHECK (is_public = true OR user_id IS NOT NULL)
);
...
BEGIN
  ...
  IF existing_constraint IS NOT NULL AND existing_constraint <> 'projects_user_id_fkey' THEN
...
-- projeto privado não pode ficar sem dono.
UPDATE public.projects
SET is_public = true
WHERE user_id IS NULL AND is_public IS DISTINCT FROM true;

ALTER TABLE public.projects
  DROP CONSTRAINT IF EXISTS projects_private_requires_owner_check;

ALTER TABLE public.projects
  ADD CONSTRAINT projects_private_requires_owner_check
...
-- =========================================
-- FEEDBACK DE EXCLUSÃO DE CONTA
-- Mantido permanentemente para análise futura,
-- mesmo após a remoção do usuário do auth.
-- =========================================
...
CREATE TABLE IF NOT EXISTS public.account_deletion_feedback (
  ...
  private_projects_deleted_count integer NOT NULL DEFAULT 0,
  public_projects_detached_count integer NOT NULL DEFAULT 0,
  stripe_subscription_id text,
  stripe_subscription_canceled boolean NOT NULL DEFAULT false,
  ...
  CONSTRAINT account_deletion_feedback_private_projects_check CHECK (private_projects_deleted_count >= 0),
  CONSTRAINT account_deletion_feedback_public_projects_check CHECK (public_projects_detached_count >= 0)
);
...
ALTER TABLE public.account_deletion_feedback
  ...
  ADD COLUMN IF NOT EXISTS private_projects_deleted_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS public_projects_detached_count integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS stripe_subscription_id text,
  ADD COLUMN IF NOT EXISTS stripe_subscription_canceled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();

UPDATE public.account_deletion_feedback
SET metadata = COALESCE(metadata, '{}'::jsonb)
WHERE metadata IS NULL;

UPDATE public.account_deletion_feedback
SET private_projects_deleted_count = COALESCE(private_projects_deleted_count, 0)
WHERE private_projects_deleted_count IS NULL;

UPDATE public.account_deletion_feedback
SET public_projects_detached_count = COALESCE(public_projects_detached_count, 0)
WHERE public_projects_detached_count IS NULL;

ALTER TABLE public.account_deletion_feedback
  ALTER COLUMN reason SET NOT NULL,
  ALTER COLUMN private_projects_deleted_count SET NOT NULL,
  ALTER COLUMN public_projects_detached_count SET NOT NULL,
  ALTER COLUMN stripe_subscription_canceled SET NOT NULL,
  ALTER COLUMN metadata SET NOT NULL,
  ALTER COLUMN created_at SET NOT NULL;

ALTER TABLE public.account_deletion_feedback
  DROP CONSTRAINT IF EXISTS account_deletion_feedback_reason_check,
  DROP CONSTRAINT IF EXISTS account_deletion_feedback_private_projects_check,
  DROP CONSTRAINT IF EXISTS account_deletion_feedback_public_projects_check;

ALTER TABLE public.account_deletion_feedback
  ADD CONSTRAINT account_deletion_feedback_reason_check CHECK (
    reason IN (
      'not_found',
      'difficult_to_use',
      'alternative_tool',
  ...
  ADD CONSTRAINT account_deletion_feedback_private_projects_check CHECK (private_projects_deleted_count >= 0),
  ADD CONSTRAINT account_deletion_feedback_public_projects_check CHECK (public_projects_detached_count >= 0);
...
CREATE INDEX IF NOT EXISTS account_deletion_feedback_email_idx
  ON public.account_deletion_feedback(email);

ALTER TABLE public.account_deletion_feedback ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "account_deletion_feedback_service_only_select" ON public.account_deletion_feedback;
DROP POLICY IF EXISTS "account_deletion_feedback_service_only_insert" ON public.account_deletion_feedback;
DROP POLICY IF EXISTS "account_deletion_feedback_deny_authenticated_select" ON public.account_deletion_feedback;

-- Sem políticas para authenticated/anon.
-- O backend usa service_role e ignora RLS.

REVOKE ALL ON public.account_deletion_feedback FROM anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.account_deletion_feedback TO service_role;
